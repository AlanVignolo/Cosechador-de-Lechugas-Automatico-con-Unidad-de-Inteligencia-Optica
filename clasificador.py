...
class GroupStatisticsAnalyzer:
    ...
    def create_summary_table(self, output_path):
        ...
        with open(txt_path, 'w', encoding='utf-8') as f:
            ...
            for group_name, data in self.group_data.items():
                for metric in ['green_pixels', 'black_pixels', 'total_pixels']:
                    all_means.append(data[metric]['mean'])
                    all_stds.append(data[metric]['std'])
            
            global_min_mean = min(all_means)
            global_max_mean = max(all_means)
            global_min_std = min(all_stds)
            global_max_std = max(all_stds)
            
            f.write(f"{'═'*80}\n")
            f.write(f"VALORES GLOBALES (TODOS LOS GRUPOS)\n")
            f.write(f"{'═'*80}\n")
            f.write(f"Media mínima encontrada:         {global_min_mean:>10.2f}\n")
            f.write(f"Media máxima encontrada:         {global_max_mean:>10.2f}\n")
            f.write(f"Desv.Est. mínima encontrada:     {global_min_std:>10.2f}\n")
            f.write(f"Desv.Est. máxima encontrada:     {global_max_std:>10.2f}\n")
    ...
    def export_json(self, output_path, separability):
        ...
        for metric, pairs in separability.items():
            ...
            for pair, stats in pairs.items():
                separability_serializable[metric][pair] = {
                    ...
                    'well_separated': bool(stats['well_separated']),
        ...
        export_data = {
            'valores_globales': {
                'media_minima': float(min(all_means)),
                'media_maxima': float(max(all_means)),
                'desviacion_minima': float(min(all_stds)),
                'desviacion_maxima': float(max(all_stds))
            },
            'grupos': {},
