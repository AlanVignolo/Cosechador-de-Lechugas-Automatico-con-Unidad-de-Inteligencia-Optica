...
El torque requerido para mover una carga mediante una varilla trapezoidal se compone de dos componentes: el torque necesario para vencer la fricción en la rosca y el torque para vencer la fricción en el collarín del soporte.

El torque para subir una carga se calcula como:
\begin{equation}
T_{\text{subida}} = \frac{F \cdot d_m}{2} \cdot k_R + \frac{F \cdot \mu_c \cdot d_c}{2}
\label{eq:torque_subida}
\end{equation}

El torque para bajar una carga se calcula como:
\begin{equation}
...
donde:
\begin{itemize}
    \item $F = 1.2\,\text{kg} \cdot 9.81\,\text{m/s}^2 = 11.77$\,N (carga a mover)
    \item $d_m = 14$\,mm (diámetro medio de la varilla TR16$\times$4)
    \item $d_c \approx 26$\,mm (diámetro medio del collarín/tuerca)
    \item $\mu_c = 0.2$ (coeficiente de fricción acero-acero para rosca trapezoidal lubricada)
    \item $k_R = 1.3$ (factor de rosca para subida, incluye efecto de cuña y fricción)
    \item $k_L = 0.6$ (factor de rosca para bajada)
\end{itemize}
